...
Future<List<IncomeModel>> getIncomes(
  ...
  if (response.statusCode == 200) {
    ...
    map.forEach((key, value) => {ccObjs.add(IncomeModel.fromJson(value, key))});
...
Future<double> getIncomesTotalSum([DateTime givenDate]) async {
  ...
  for (var account in accounts) {
    totalAmount += await getIncomesSum(account.id, month, year);
  }

  return totalAmount;
}

Future<String> saveIncome(double amount, String currency, String description,
...
Future<String> deleteIncome(IncomeModel income) async {
  ...
  MoskaCacheManager().removeFile(url +
      "/byAccount/${income.accountId}?month=${income.date.month}&year=${income.date.year}");
  MoskaCacheManager().removeFile(
      accountsUrl);
